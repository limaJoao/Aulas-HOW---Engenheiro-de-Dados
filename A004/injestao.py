...
# Criando uma classe para pegar os bitcoins
...
# Seguindo o principio aberto fechado criando uma nova classe para pegar o day-summary
...
class TradesApi(MercadoBitcoinApi):
    ...
    def _get_date_epoch(self, date: datetime.datetime) -> int:
    ...
    def _get_endpoint(self, date_from: datetime.datetime = None, date_to: datetime.datetime = None) -> str:
        ...
        if date_from and not date_to:
            unix_date_from = self._get_date_epoch(date_from)
            endpoint = f"{self.base_endpoint}/{self.coin}/{self.type}/{unix_date_from}"

        elif date_from and date_to:
            unix_date_from = self._get_date_epoch(date_from)
            unix_date_to = self._get_date_epoch(date_to)
...
print(TradesApi(coin='BTC').get_data(date_from = datetime.datetime(2021,6,2), 
                                     date_to = datetime.datetime(2021,6,3)))
...
data = DaySummaryApi(coin='BTC').get_data(date=datetime.date(2021,6,20))
...
class DataIngestor(ABC):
    ...
    @property
    def _checkpoint_filename(self) -> self:
...
ingestor=DaySummaryIngestor(writer=DataWriter, coins=['BTC','ETH','LTC'],
